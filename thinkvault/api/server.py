...
async def verify_api_token(
    ...
    """API Token 认证依赖。TOKEN 未配置时放行全部请求。"""
    if not THINKVAULT_API_TOKEN:
        return True
    if credentials is None:
        raise HTTPException(status_code=401, detail="未提供认证令牌，请在 Authorization header 中传入 Bearer <token>")
    if not secrets.compare_digest(credentials.credentials, THINKVAULT_API_TOKEN):
    ...
    async def rate_limit_middleware(request: Request, call_next):
        ...
        with _rate_limit_lock:
            ...
            if len(timestamps) >= _RATE_LIMIT_REQUESTS:
                raise HTTPException(
                    status_code=429,
                    detail=f"请求过于频繁，请 {_RATE_LIMIT_WINDOW}s 后重试",
                )
